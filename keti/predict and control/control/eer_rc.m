...
clc
%从Excel文件导入样本数据     +数字滤波
A=xlsread('D:\研究生\研二\项目资料\程序\XZM 扬州智谷\XZMmodel eer+制冷量\副本input(2019).xlsx');
A=smoothdata(A,'gaussian',30)
%A=smoothts(A,'b',30);
C=A';%转置
%C = smoothdata(C)
%C=smoothts(B,'b',20)
a=xlsread('D:\研究生\研二\项目资料\程序\XZM 扬州智谷\XZMmodel eer+制冷量\副本output(2019).xlsx');   
a=smoothdata(a,'gaussian',30) 
...
E_MEAN1=E1/677                                 %误差绝对平均值

figure(1);
plot(Error1);
title('EER output error');
xlabel('number)');
ylabel('Error');
...
% figure(4);
% plot(T_real(2,:));
% title('Qfuhetest output compare');
% xlabel('number');
% ylabel('Qfuhe');
% hold on
% plot(T_test(2,:),'r+');
